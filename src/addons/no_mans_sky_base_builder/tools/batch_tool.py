...
class BatchTool(bpy.types.PropertyGroup):
    ...
    def batch_replace_with_target_object(self):
        ...
        if self.nms_batch_replace_type == "target" and not self.target_object:
            ...
            ShowMessageBox( message= message, title=title )
            return 0
        elif self.nms_batch_replace_type == "target" and self.object_id not in nice_name_dictionary:
            title="Batch Replace Objects"
        ...
        for source_object in selected_objects:
            ...
            replaced_object = blend_utils.duplicate_part(target_object)
            replaced_object.location = source_object.location.copy()
            replaced_object.scale = source_object.scale.copy()
            replaced_object.rotation_euler = source_object.rotation_euler.copy()
            
            blend_utils.delete(source_object)
            replaced_objects_list.append(replaced_object)
            
        if self.nms_batch_replace_type == "object_id":
            blend_utils.delete(target_object)
