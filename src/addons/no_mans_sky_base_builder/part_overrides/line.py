...
from .. import part
from ..utils import blend_utils, constraints, material


class Line(part.Part):
    def __init__(self, bpy_object=None, build_rigs=True, *args, **kwargs):
        super(Line, self).__init__(
            bpy_object=bpy_object, build_rigs=build_rigs, *args, **kwargs
        )
    ...
    def build_rig(self, start=None, end=None):
        ...
        """
        # Safely compose the matrix directly from local transforms, bypassing stale depsgraph
        world_pos = mathutils.Matrix.LocRotScale(
            self.object.location, self.object.rotation_euler, self.object.scale
        )
        
        # Account for parent offsets if any exist
        if self.object.parent:
            world_pos = self.object.parent.matrix_world @ self.object.matrix_parent_inverse @ world_pos

        world_loc = world_pos.decompose()[0]
        if not start:
            start = Line.create_point(
                self.builder, name="_".join([self.object.name, "START"])
            )
        ...
        if not end:
                ...
                builder=self.builder, name="_".join([self.object.name, "END"])
            )
            # Snap the end location to the end of power line.
            at_vec = mathutils.Vector(
                [world_pos[0][2], world_pos[1][2], world_pos[2][2]]
            )
    ...
    def split(self):
        ...
        # Snap the middle locations mathematically using LERP
        middle_control_a.location = prev_start_control.location.lerp(prev_end_control.location, 0.4)
        middle_control_b.location = prev_start_control.location.lerp(prev_end_control.location, 0.6)
        ...
        # Select the middle controller safely (from previous fix)
        try:
    ...
    def divide(self):
        ...
        # Snap the middle location mathematically using LERP
        middle_control.location = prev_start_control.location.lerp(prev_end_control.location, 0.5)
    ...
    def deserialise_from_data(
        cls, data, builder_object, build_rigs=True, compensate_normal=True
    ):
        ...
        part = cls(
            object_id=object_id, builder_object=builder_object, build_rigs=build_rigs
        )
        # if not build_rigs:
    ...
    def create_point(builder, name=None):
        ...
        if existing_object:
            ...
            with bpy.data.libraries.load(blend_path, link=False) as (
                ...
                data_to.objects = [
                    scene_name
                    for scene_name in data_from.objects
                    if scene_name.startswith(obj_name)
                ]
        ...
        n = 0
        base_name = name
        while point.name != name and (
            name == default_name or blend_utils.item_exists_by_name(name)
        ):
            n += 1
    ...
    def position_matrix(data, bpy_object, compensate_normal=True):
        ...
        # Set part position.
        world_matrix = Line.create_matrix_from_vectors(
            pos, up, at, compensate_normal=compensate_normal
        )
    ...
    def generate_control_points(source, target, builder):
        ...
        source_key, target_key = source.get_closest_snap_points(
            target, source_filter="POWER", target_filter="POWER"
        )
