...
import uuid
import math
from . import blend_utils, curve_utils, collection_utils
...
def duplicate_along_curve( object, curve, number_of_duplicates=10, radius_multiplier=1.0):
    ...
    # Add additional obejcts if needed to reach desired number of objects
        ...
        child_col, curve_col = collection_utils.move_curve_to_collection(curve)
        
        if len(existing_objs) > 1:
            last_object = existing_objs[-1]
            last_rotation_euler = last_object.rotation_euler.copy()
            last_position = last_object.location.copy()
        ...
        for _ in range(number_of_duplicates - current_count):
            ...
            if child_col not in new_obj.users_collection:
                child_col.objects.link(new_obj)
...
def mirror_curve(curve_obj, axis = "Z", center = None, auto_duplicate = False):
    ...
    duplicate_along_curve(None, new_curve_obj, number_of_objects, radius_multiplier)
    sync_curves(new_curve_obj, curve_obj, True)

    return new_curve_obj


def sync_curves(target_curve, source_curve, do_mirror = False):
